 ...
 *   enablePlugin() now checks plugin.skipCache before loading from
 *   IndexedDB. This lets plugins like GeoNewsPlugin that manage their
 *   own in-memory state skip the stale-cache-on-toggle problem.
 *
 *   To opt out of caching, add to a plugin class:
...
class PluginManager {
    ...
    async enablePlugin(pluginId: string): Promise<void> {
        ...
        // ── Cache handling ─────────────────────────────────────
        // If the plugin declares skipCache = true, invalidate any
        // stale IndexedDB entry and skip straight to the live fetch.
        // This prevents old cached data from flashing before the fresh
        // DB fetch arrives (critical for GeoNewsPlugin).
        const skipCache = (managed.plugin as any).skipCache === true;

        if (skipCache) {
            cacheLayer.invalidate(pluginId);
        } else {
            // Default behaviour: show cached data immediately while
    ...
    destroy(): void {
        ...
        this.plugins.forEach((managed) => {
            try { managed.plugin.destroy(); } catch { /* ignore */ }
        });
        this.plugins.clear();
    }
