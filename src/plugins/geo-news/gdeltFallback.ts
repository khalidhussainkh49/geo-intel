 ...
 * Uses keyword classification (not LLM) since GDELT is a quick
 * inline fallback — we don't want LLM latency on every GET request.
 */

import { geocodeText }                        from "./nigeriaLocations";
import { isNigeriaRelevant, makeId }          from "./newsPipeline";
import { classifyByKeyword }                  from "./llmclassifier";
...
export async function fetchGdeltFallback(): Promise<RawNewsArticle[]> {
    try {
        ...
        for (const item of items) {
            ...
            // Use keyword classifier for GDELT fallback (fast, no LLM cost)
            const cls = classifyByKeyword(title, "");
