 ...
 *   2. Filters to Nigeria-relevant content only
 *   3. Geocodes each article to a lat/lon
 *   4. Deduplicates against existing DB records (skip already-stored articles)
 *   5. Classifies NEW articles via LLM (Claude) → keyword fallback → default
 *   6. Saves classified articles to Supabase
 *   7. Returns pipeline stats
 *
 * Classification is now done AFTER deduplication so we only spend
 * LLM tokens on genuinely new articles.
 *
 * Called by:
 *   - /api/geo-news/poll/route.ts  → Vercel cron every 5 minutes
 *   - /api/geo-news/route.ts POST  → manual trigger
...
// Classification is done later in a batch after deduplication.
...
function rssItemToStub(item: RssItem, source: NewsSource): ArticleStub | null {
...
function acledEventToStub(event: AcledEvent): ArticleStub | null {
...
export async function runNewsPipeline(options: {
    ...
    const sources = sourcesToRun
        ...
        : ALL_SOURCES.filter(s => s.id !== "acled");
    ...
    const tasks = sources.map(source => async () => {
        try {
            ...
            for (const item of items) {
                const stub = rssItemToStub(item, source);
                if (stub) stubs.push(stub);
            }

            result.bySource[source.id] = stubs.length;
            return { sourceId: source.id, stubs };
        } catch (err: any) {
            result.errors.push(`${source.id}: ${err.message}`);
            result.bySource[source.id] = 0;
            return { sourceId: source.id, stubs: [] as ArticleStub[] };
        }
    });

    const batchResults = await withConcurrency(tasks, concurrency);

    // ── Step 2: Collect all stubs ─────────────────────────────
    const allStubs: ArticleStub[] = [];
    for (const { stubs } of batchResults) {
        allStubs.push(...stubs);
    }
    ...
    if (!sourcesToRun || sourcesToRun.includes("acled")) {
        try {
            ...
            for (const event of events) {
                const stub = acledEventToStub(event);
                if (stub) allStubs.push(stub);
            }
            result.bySource["acled"] = allStubs.filter(s => s.sourceId === "acled").length;
        } catch (err: any) {
    ...
    // ── Step 5: DB deduplication — only classify NEW articles ─
    // This is the key optimisation: we don't spend LLM tokens on
    // articles that are already in the database.
    ...
    if (stubsToClassify.length === 0) {
        // Nothing new to classify or save
        result.articles = uniqueStubs.map(stub => ({
            ...stub,
    ...
    console.log(
        `[newsPipeline] Classifying ${stubsToClassify.length} new articles` +
        ` (${result.skipped} already in DB)...`
    );

    // ── Step 6: LLM classification (batched) ──────────────────
    const classificationInputs = stubsToClassify.map(s => ({
        title: s.title,
        summary: s.summary,
    }));

    const classifications = await classifyArticles(classificationInputs);

    // Count by method
    for (const c of classifications) {
        if (c.method === "llm") result.classifiedByLlm++;
        else result.classifiedByKw++;
    }

    console.log(
        `[newsPipeline] Classification done: ` +
        `${result.classifiedByLlm} LLM, ${result.classifiedByKw} keyword`
    );

    // ── Step 7: Merge stubs + classifications → RawNewsArticle ─
    const classifiedArticles: RawNewsArticle[] = stubsToClassify.map((stub, i) => {
    ...
    // ── Step 8: Save to DB ─────────────────────────────────────
    if (saveToDb && classifiedArticles.length > 0) {
        // Group by sourceId
        const bySource = new Map<string, RawNewsArticle[]>();
        for (let i = 0; i < stubsToClassify.length; i++) {
            const sid = stubsToClassify[i].sourceId;
            if (!bySource.has(sid)) bySource.set(sid, []);
            bySource.get(sid)!.push(classifiedArticles[i]);
        }
    ...
    result.articles = classifiedArticles;
    return result;
}

// Re-export classifyArticle for backward compatibility with any
// code that calls it directly (e.g. gdeltFallback.ts)
export { classifyByKeyword as classifyArticle };
